...
const deleteFavorite = async (wordId) => {
    ...
    const url = `http:/54.66.160.176:3000/deleteFavorite?wordId=${wordId}` ;
...
const queryFavorite = async (wordId) => {
    ...
    const url = `http://54.66.160.176:3000/queryFavorite?wordId=${wordId}` ;
    const response = await fetch(url, {
        method : "GET",
        headers : {'Content-Type': 'application/json'},
    }) ;
...
const addFavorite = async(wordId) => {
    ...
    const url = `http://54.66.160.176:3000/addFavorite?wordId=${wordId}` ;
    const response = await fetch(url, {
        method : "GET",
        headers : {'Content-Type': 'application/json'},
    }) ;
...
const updateWords = async (category) => {
    if (categorySelect.value === "" || chapterSelect.value === ""){
        ...
        if (category === "favorite"){
            ...
            url = `http://54.66.160.176:3000/getFavoriteWords` ;
        }else{
            // url = `https://kimery.store/getWords?category=${categorySelect.value}&chapter=${chapterSelect.value}` ;
            url = `http://54.66.160.176:3000/getWords?category=${categorySelect.value}&chapter=${chapterSelect.value}` ;
        }
        const response = await fetch(url, {
            method : "GET",
            headers : {'Content-Type': 'application/json'},
